...
export function ImagePickerModal({ nodeId, assetId, onClose }: Props) {
  ...
  const canvasImages = nodes
    .filter(
      (n): n is Extract<typeof n, { kind: "image" | "generateImage" }> =>
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (result) selectImage(result.url);
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Tabs */}
        <div className="flex gap-1 flex-shrink-0" style={{ padding: "16px 24px 0" }}>
          {TABS.map((t) => (
            <button
              key={t.key}
                ...
                <div className="flex flex-col items-center gap-2">
                  {uploading ? (
                    <>
                      ...
                      <span className="text-[14px]">上传中…</span>
                    </>
                  ) : (
                    <>
                      <span className="text-[32px]">+</span>
                      <span className="text-[14px]">点击选择本地图片</span>
                    </>
                  )}
                </div>
              </button>
