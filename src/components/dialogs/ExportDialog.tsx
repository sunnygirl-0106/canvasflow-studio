    ...
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-6 fade-in">
      <div className="bg-card border border-border rounded-2xl w-full max-w-2xl p-5 node-shadow">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={() => setExport(false)} className="text-muted-foreground hover:text-foreground">
            <X className="w-4 h-4" />
          </button>
        </div>
        <textarea
        ...
        <div className="mt-3 flex items-center justify-between">
          <div className="text-xs text-muted-foreground">演示模式 · 文件已"下载"到 ~/Downloads (mock)</div>
          <div className="flex gap-2">
            <button onClick={copy} className="flex items-center gap-1.5 text-xs px-3 py-2 rounded-md bg-secondary hover:bg-secondary/70">
              {copied ? <><Check className="w-3.5 h-3.5 text-accent" />已复制</> : <><Copy className="w-3.5 h-3.5" />复制</>}
            </button>
            <button onClick={() => setExport(false)} className="flex items-center gap-1.5 text-xs px-3 py-2 rounded-md bg-primary text-primary-foreground">
              <Download className="w-3.5 h-3.5" />确认下载
            </button>
          </div>
        </div>
      </div>
...
function buildFcpxml(shots: any[], name: string) {
  let cur = 0;
