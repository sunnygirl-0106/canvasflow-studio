...
export function GroupNode({ data }: { id: string; data: CanvasNode["data"] }) {
  ...
  const isImageGroup = !isVideoGroup && members.length > 0 && members.every((m) => m.kind === "image" && m.src);
  ...
  return (
    <div
      ...
      style={{
        ...
        border: isVideoGroup || isImageGroup ? `2px solid ${isImageGroup ? color : "#E2E8F0"}` : `2px dashed ${color}`,
    ...
    >
            ...
            {members.map((m) => (
              ...
              >
                ...
                <div style={{ padding: "6px 8px" }}>
                  <span className="text-[11px] font-medium truncate block" style={{ color: "#334155" }}>
                    {m.name}
                  </span>
                </div>
              </div>
      ...
      <Handle type="target" position={Position.Left} id="group-in" style={{ background: isVideoGroup ? "#14B8A6" : color }} />
      <Handle type="source" position={Position.Right} id="group-out" style={{ background: isVideoGroup ? "#14B8A6" : color }} />
    </div>
  );
}
