...
export function createCompositionSlice(set: SetState, get: GetState) {
  return {
    ...
    mergeToComposition: (nodeIds: string[]) => {
      ...
      const videoPicked = picked.filter((n) => videoKinds.includes(n.kind));
      const audioPicked = picked.filter((n) => n.kind === "audio");
    ...
    moveClip: (compId: string, clipId: string, toTrackId: string, toStartSec: number) => {
      ...
      set((s) => {
        ...
        if (!toTrack) return s;
        if (sourceClip.clipKind !== toTrack.kind) return s;
