import { generateStoryboardScript } from "@/services/scriptGen";
...
function patchScriptData(
  ...
  return nodes.map((n) => {
    if (n.id !== id || !n.data.script) return n;
  ...
  return {
    ...
    generateScript: (id: string) => {
      ...
      generateStoryboardScript(
        { sourceText: srcText, promptText: sb.promptText, model: sb.model },
        {
          ...
          onDone: () =>
            set((s) => ({ nodes: patchScriptData(s.nodes, id, { status: "ready", progress: undefined }) })),
          onError: (err) =>
            set((s) => ({
              nodes: patchScriptData(s.nodes, id, { status: "failed", error: err.message, progress: undefined }),
            })),
    ...
    toggleScriptColumn: (id: string, key: ScriptColumnKey) =>
      set((s) => {
        ...
        const hidden = node?.data.script?.hiddenColumns ?? [];
    ...
    setScriptFilter: (id: string, filter: Partial<ScriptFilter>) =>
      set((s) => {
        const node = s.nodes.find((n) => n.id === id);
        const current = node?.data.script?.filter ?? {};
    ...
    addAsset: (id: string, asset: ScriptAsset) =>
      set((s) => {
        const node = s.nodes.find((n) => n.id === id);
        const assets = [...(node?.data.script?.assets ?? []), asset];
    ...
    updateAsset: (id: string, assetId: string, patch: Partial<ScriptAsset>) =>
      set((s) => {
        const node = s.nodes.find((n) => n.id === id);
        const assets = (node?.data.script?.assets ?? []).map((a) =>
    ...
    removeAsset: (id: string, assetId: string) =>
      set((s) => {
        const node = s.nodes.find((n) => n.id === id);
        const assets = (node?.data.script?.assets ?? []).filter((a) => a.id !== assetId);
        return { nodes: patchScriptData(s.nodes, id, { assets }) };
      }),
